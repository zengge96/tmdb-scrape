...
### Step 2: 批次执行逻辑（通过Subagent执行）
针对每个批次，启一个Subagent，注意只启一个Subagent，不要多个并发，否则会触发API 429错误，按顺序执行以下任务：

1. **读取批次**: 调用 `node get_one_bach_lines.js` 读取批次。
   * **逻辑**: 从带行号的文件读取一个批次，每个文件建议 30 行，以控制单次 AI 处理的 Token 长度。
   * **输入**: `temp/indexed.txt`带行号的文件, 批次编号, 每批次行数。
   * **输出**: 打印到控制台，本批次的所有行，格式：行号#路径
   * **注意**: 每个批次循环开始时读取一次，不要一次性预读取多批，也不要把读取内容存入临时文件。

2. **AI 提取**: 从原始路径中智能提取**纯净片名**和**年份**（处理路径乱码、去除字幕组标签等）。
   * 格式化为：`行号#全路径名#纯净片名#年份`

3. **脚本搜索**: 调用 `node tmdb_search.js` 获取候选列表。
   * **逻辑**: 脚本内部进行年份容差过滤 (±1 年)。
   * **输入**: `temp/batch-XX-clean.txt`
   * **输出**: `temp/batch-XX-search.txt` (含候选结果集)
   * **注意**: 搜索结果为空时直接记录至错误文件，不进入下一步。

4. **⚠️ AI 验证选择（必做）**: 从搜索结果候选中选出最匹配的 `TMDB_ID`。
   * **必须执行，不可跳过**
   * 输入: 搜索结果文件（包含多个候选的数组）
   ...
   * **选择规则说明**: 
   * 根据路径中的片名相关性选择，如果没有相关的结果则不选（即选中结果为空{}），宁可返回空也不要误匹配。
   * 这里不用考虑年份容差，因为搜索脚本已经考虑过了。

5. **脚本详情**: 调用 `node tmdb_details.js` 获取国家、评分、海报等详细元数据。

6. **脚本映射**: 调用 `node map_path.js` 将 API 结果与原始路径关联，生成最终行数据。
...
# 2. 循环处理批次 (以批次01为例)
node get_one_bach_lines.js indexed.txt 1 30

# [AI 步骤]: 从get_one_bach_lines.js 控制台输出提取片名/年份 -> 存入 batch-01-clean.txt
##### **重要提醒：纯AI提取，不得使用脚本，也不要生成脚本提取**
##### * 1、影片名：从路径中提取，尽量不要翻译（除非你100%确定是官方译名），如果路径中同时存在多种语言片名，中文优先*
##### * 2、年份：从路径中提取，如果路径中没有年份或年份不明确（例如：2018-2022之间），保持为空（例如：656#./🏷️我的115分享/千部抖音短剧合集/613.神医小师弟#神医小师弟#）*
